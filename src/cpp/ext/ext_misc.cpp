  ...
  if ((colon = (char*)memchr(data, ':', len)) && colon[1] == ':') {
    ...
    if (f_class_exists(className)) { // taking care of volatile class
      ...
      for (const char *parentClass = className.data(); parentClass;
        ...
        ClassInfo::ConstantInfo *cinfo = info->getConstantInfo(constantName);
        // This isn't right, only constants that have scalar values are valid
...
Variant f_time_nanosleep(int seconds, int nanoseconds) {
  if (seconds < 0) {
    throw InvalidArgumentException("seconds", "cannot be negative");
  }
  if (nanoseconds < 0 || nanoseconds > 999999999) {
    throw InvalidArgumentException("nanoseconds", "has to be 0 to 999999999");
  }
